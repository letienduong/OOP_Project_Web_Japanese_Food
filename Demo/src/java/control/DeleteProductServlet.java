...
public class DeleteProductServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        DAO dao = new DAO();
        boolean isDeleted; // Giả sử phương thức deleteProduct trả về true/false
        isDeleted = dao.deleteProduct(id);
        
        // Thiết lập loại nội dung cho phản hồi là JSON
        response.setContentType("application/json");
        PrintWriter out = response.getWriter();
        
        // Trả về kết quả dưới dạng JSON
        if (isDeleted) {
            out.write("{\"success\": true, \"message\": \"Sản phẩm đã được xóa thành công!\"}");
        } else {
